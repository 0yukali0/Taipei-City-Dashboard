...
class CHTAuth:
    ...
    def get_token(self, now_time, is_proxy=False, timeout=10):
            ...
            with open(self.full_file_path, "rb") as handle:
                ...
                if isinstance(res["time_out"], str):
                    time_out = datetime.strptime(
                        res["time_out"], "%Y-%m-%d %H:%M:%S"
                    ).replace(tzinfo=taiwan_timezone)
                else:
        ...
        payload = json.dumps({"id": self.account, "pass": self.password})
        ...
        with requests.post(
            ...
            res = {
                "access_token": token,
                "time_out": time_out_plus_30.strftime("%Y-%m-%d %H:%M:%S"),
            }

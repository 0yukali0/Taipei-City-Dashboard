...
def _transfer(**kwargs):
    ...
    data["type"] = "parking"
    data["disabled_parking_car_count"] = data["charged"].apply(
        lambda x: parse_charged(x, "身汽")
    )
    data["disabled_parking_motorcycle_count"] = data["charged"].apply(
        lambda x: parse_charged(x, "身機")
    )
...
dag = CommonDag(
    proj_folder="proj_new_taipei_city_dashboard", dag_folder="accessible_facilities"
)

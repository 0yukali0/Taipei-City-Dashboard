...
def _transfer(**kwargs):
    ...
    data = pd.concat(
        [
            data.drop(["StationName", "StationPosition"], axis=1),
            data["StationName"].apply(pd.Series),
            data["StationPosition"].apply(pd.Series),
        ],
        axis=1,
    )
...
dag = CommonDag(
    ...
    dag_folder="youbike_station_realtime_usage_and_availability",
)

    ...
    with data_engine.connect() as connection:
        ...
        if not unique_names:
            try:
                    ...
                    for rec in recs:
                        ...
                        if dash_recs:
                            for dash in dash_recs:
                                ...
                                print(
                                    f"Deleted dashboard id={dash_id} for component {comp_id}"
                                )
                            # 刪除 component
                            dashboard_hook.run(
                ...
                try:
                    ...
                    pass
                print(
                    "已清除所有 disaster_sus_*_% 相關 dashboard、groups、components、charts"
                )
        ...
        for pname in unique_names:
            ...
            for status_key, status_val in status_mapping.items():
                ...
                comp_id = recs[0][0]
                print(
                    f"已建立或確認 component id={comp_id}, index={comp_index}, name={comp_name}"
                )
                ...
                if not df.empty:
                                ...
                                if isinstance(val, (dict, list)):
                                    ...
                                    if (
                                        ...
                                        try:
                                            ...
                                            print(
                                                f"Warning: Failed to parse column '{col}': {val}"
                                            )
                                            return None
                                return val
                    ...
                    with pg_engine.begin() as conn:
                        conn.execute(
                            text(
                                'DELETE FROM public.query_charts WHERE "index" = :idx'
                            ),
                            {"idx": f"{status_key}_{pname}"},
                        )
                    ...
                    print(
                        f"已用 DataFrame upsert query_charts index={status_key}_{pname}"
                    )
                ...
                if not df_chart_template.empty:
                    ...
                    if "types" in df_chart_template.columns:
                        ...
                        df_chart_template["types"] = df_chart_template["types"].apply(
                            to_py_list
                        )
                    ...
                    with pg_engine.begin() as conn:
                        ...
                        conn.execute(
                            text(
                                'DELETE FROM public.component_charts WHERE "index" = :idx'
                            ),
                            {"idx": new_chart_index},
                        )
                        # append 新資料
                        df_chart_template.to_sql(
                            "component_charts",
                            conn,
                            if_exists="append",
                            index=False,
                    ...
                    print(
                        f"已用 DataFrame upsert component_charts index={new_chart_index}"
                    )
                # --- component_charts 修改結束 ---

            # --- 修改取得 component id 的方式 ---
            # 產生所有需要的 component index
            comp_indices_to_fetch = [
                f"{status_key}_{pname}" for status_key in status_mapping.keys()
            ]

            # 使用 get_pandas_df 一次取得所有 component id
            if comp_indices_to_fetch:
                sql_get_ids = (
                    'SELECT id FROM public.components WHERE "index" = ANY(%(indices)s);'
                )
                df_comp_ids = dashboard_hook.get_pandas_df(
                    sql=sql_get_ids, parameters={"indices": comp_indices_to_fetch}
                )
            ...
            print(
                f"已建立/更新 dashboard: idx={rand_idx}, name={pname}, components={comp_ids}"
            )
            ...
            print(
                f"已建立 dashboard_groups 關聯: dashboard_id={dashboard_id}, group_id={group_id}"
            )
...
dag = CommonDag(
    proj_folder="proj_city_dashboard", dag_folder="eoc_dashboard_controller"
)

...
def _transfer(**kwargs):
    ...
    res = get_data_taipei_api(
        get_current_rid_from_page_id(page_id, resource_name_contains="查核成效")
    )
    ...
    data["reinspection_compliance_rate"] = data["reinspection_compliance_rate"].replace(
        "%", "", regex=True
    )
    ...
    for col in numeric_cols:
        data[col] = pd.to_numeric(
            data[col].replace(",", "", regex=True), errors="raise"
        ).astype(int)
    # Time
...
dag = CommonDag(
    ...
    dag_folder="energy_saving_coaching_situation_tpe_gov",
)

...
def _transfer(**kwargs):
    ...
    area_map = raw_data[raw_data["district_code"].str.len() == 8][
        ["district_code", "district"]
    ]
    area_map = (
        area_map.drop_duplicates().set_index("district_code")["district"].to_dict()
    )
    ...
    raw_data["age_100_plus"] = pd.to_numeric(
        raw_data["age_100_plus"], errors="coerce"
    ).fillna(0)
    ...
    save_dataframe_to_postgresql(
        engine, data=melt_df, load_behavior=load_behavior, default_table=default_table
    )

...
def _transfer(**kwargs):
    ...
    # Extract
    res = get_data_taipei_api(
        get_current_rid_from_page_id(page_id, resource_name_contains="輔導成效")
    )
    ...
    data["year"] = (
        data["year"].astype(str).str.extract(r"(\d+)", expand=False).astype(int) + 1911
    )
    data["power_saving_degree"] = (
        data["power_saving_degree"]
        .replace("萬", "", regex=True)
        .replace("", "", regex=True)
        .replace(",", "", regex=True)
    )
    data["power_saving_degree"] = data["power_saving_degree"].astype(int)
    data["electricity_saving_expenses"] = (
        data["electricity_saving_expenses"]
        .replace("萬元", "", regex=True)
        .replace("", "", regex=True)
        .replace(",", "", regex=True)
    )
    data["electricity_saving_expenses"] = data["electricity_saving_expenses"].astype(
        int
    )
    data["carbon_reduction_metric_tons"] = (
        data["carbon_reduction_metric_tons"]
        .replace("", "", regex=True)
        .replace(",", "", regex=True)
    )
    data["carbon_reduction_metric_tons"] = data["carbon_reduction_metric_tons"].astype(
        int
    )
    data["carbon_sink_daan_forest_parks"] = data[
...
dag = CommonDag(
    proj_folder="proj_city_dashboard", dag_folder="energy_saving_coaching_situation_tpe"
)

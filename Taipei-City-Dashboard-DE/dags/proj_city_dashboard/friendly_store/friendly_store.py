...
def _transfer(**kwargs):
    ...
    res = get_data_taipei_api(
        get_current_rid_from_page_id(page_id, resource_name_contains="友善店家")
    )
    ...
    gdata = add_point_wkbgeometry_column_to_df(
        raw_data, raw_data["lon"], raw_data["lat"], from_crs=FROM_CRS
    )
    ...
    update_lasttime_in_data_to_dataset_info(
        engine, airflow_dag_id=dag_id, lasttime_in_data=raw_data["data_time"].max()
    )
